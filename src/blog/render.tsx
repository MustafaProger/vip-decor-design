...
import App from "../App";
import { blogMetadata, blogPage, blogSchema } from "./content";
const escape = (s: string) =>
  s
    ...
    .replaceAll(">", "&gt;")
    .replaceAll('"', "&quot;");
export function renderBlogDocument(url: string, template: string) {
  ...
  const head =
    ...
    Object.entries({
      ...
      "og:type": blogPage(path).post ? "article" : "website",
      "og:locale": "ru_RU",
    })
      .map(
        ([property, content]) =>
      ...
      .join("\n") +
    `<script data-blog-meta type="application/ld+json">${JSON.stringify(blogSchema(path)).replaceAll("<", "\\u003c")}</script>`;
  return template
